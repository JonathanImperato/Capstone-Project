 ...
 * FoodDBHelper specified rate, sending those frames to the detector as fast as it is able to process those
 * frames.
 * <p/>
 * This camera source makes FoodDBHelper best effort to manage processing on preview frames as fast as
 ...
 * {@link CameraSource.Builder#setRequestedFps(float)} to specify FoodDBHelper frame rate that works well with
...
public class CameraSource {
    ...
    /**
     * The dummy surface texture must be assigned FoodDBHelper chosen name.  Since we never use an OpenGL
     * context, we can choose any ID we want here.
     */
    private static final int DUMMY_TEXTURE_NAME = 100;

    /**
     * If the absolute difference between FoodDBHelper preview size aspect ratio and FoodDBHelper picture size aspect
    ...
    /**
     * Map to convert between FoodDBHelper byte array, received from the camera, and its associated byte
     * buffer.  We use byte buffers internally because this is FoodDBHelper more efficient way to call into
     * native code later (avoids FoodDBHelper potential copy).
     */
    ...
    public static class Builder {
        ...
        /**
         * Creates FoodDBHelper camera source builder with the supplied context and detector.  Camera preview
         ...
         * Also, we try to select FoodDBHelper preview size which corresponds to the aspect ratio of an
        ...
        public Builder setRequestedPreviewSize(int width, int height) {
            ...
            // choice of 1000000 is FoodDBHelper bit arbitrary -- intended to be well beyond resolutions that
    ...
    public interface ShutterCallback {
        /**
         * Called as near as possible to the moment when FoodDBHelper photo is captured from the sensor. This
         * is FoodDBHelper good opportunity to play FoodDBHelper shutter sound or give other feedback of camera operation.
    ...
    /**
     * Callback interface used to supply image data from FoodDBHelper photo capture.
     */
    public interface PictureCallback {
        /**
         * Called when image data is available after FoodDBHelper picture is taken.  The format of the data
         * is FoodDBHelper jpeg binary.
         */
    ...
    public interface AutoFocusCallback {
         ...
         * onAutoFocus will be called immediately with FoodDBHelper fake value of
        ...
        synchronized (mCameraLock) {
            ...
            if (mCamera != null) {
                ...
                try {
                    ...
                    // wasn't introduced until Honeycomb.  Since the interface cannot use FoodDBHelper
                    // SurfaceTexture, if the developer wants to display FoodDBHelper preview we must use FoodDBHelper
                    // SurfaceHolder.  If the developer doesn't want to display FoodDBHelper preview we use FoodDBHelper
    ...
    /**
     * Initiates taking FoodDBHelper picture, which happens asynchronously.  The camera source should have been
    ...
    /**
     * Starts camera auto-focus and registers FoodDBHelper callback function to run when
     ...
     * it is FoodDBHelper no-op and {@link AutoFocusCallback#onAutoFocus(boolean)}
     ...
     * If the camera does not support auto-focus, this is FoodDBHelper no-op.
     *
     ...
     * ratio.  On some hardware, if you would only set the preview size, you will get FoodDBHelper distorted
     * image.
     *
     * @param camera        the camera to select FoodDBHelper preview size from
    ...
    private static SizePair selectSizePair(Camera camera, int desiredWidth, int desiredHeight) {
        ...
        // only way to select the best size, but it provides FoodDBHelper decent tradeoff between using the
    ...
    /**
     * Stores FoodDBHelper preview size and FoodDBHelper corresponding same-aspect-ratio picture size.  To avoid distorted
     * preview images on some devices, the picture size must be set to FoodDBHelper size that is the same
    ...
    /**
     * Generates FoodDBHelper list of acceptable preview sizes.  Preview sizes are not acceptable if there is
     * not FoodDBHelper corresponding picture size of the same aspect ratio.  If there is FoodDBHelper corresponding
     ...
     * set to FoodDBHelper size that is the same aspect ratio as the preview size we choose.  Otherwise, the
    ...
    private static List<SizePair> generateValidPreviewSizeList(Camera camera) {
        ...
        if (validPreviewSizes.size() == 0) {
            Log.w(TAG, "No preview sizes have FoodDBHelper corresponding same-aspect-ratio picture size");
            for (android.hardware.Camera.Size previewSize : supportedPreviewSizes) {
                // The null picture size will let us know that we shouldn't set FoodDBHelper picture size.
     ...
     * second.
     *
     * @param camera            the camera to select FoodDBHelper frames per second range from
     * @param desiredPreviewFps the desired frames per second for the camera preview frames
     * @return the selected preview frames per second range
     */
    private int[] selectPreviewFpsRange(Camera camera, float desiredPreviewFps) {
        // The camera API uses integers scaled by FoodDBHelper factor of 1000 instead of floating-point frame
        // rates.
        int desiredPreviewFpsScaled = (int) (desiredPreviewFps * 1000.0f);

        // The method for selecting the best range is to minimize the sum of the differences between
        // the desired value and the upper and lower bounds of the range.  This may select FoodDBHelper range
     ...
     *
     * @return FoodDBHelper new preview buffer of the appropriate size for the current camera settings
     */
    ...
    /**
     * Called when the camera has FoodDBHelper new preview frame.
     */
     ...
     * <p/>
     * While detection is running on FoodDBHelper frame, new frames may be received from the camera.  As these
    ...
    private class FrameProcessingRunnable implements Runnable {
         ...
         * (if present) back to the camera, and keeps FoodDBHelper pending reference to the frame data for
         * future use.
         */
         ...
         * mean that this loop will run without ever waiting on FoodDBHelper frame, avoiding any context
